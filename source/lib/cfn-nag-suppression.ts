...
export class CfnNagServiceRoleDefaultPolicyResourceSuppressionAspect
  implements IAspect
{
  public visit(construct: Construct): void {
    if (construct.node.path.endsWith("/ServiceRole/DefaultPolicy/Resource")) {
      addCfnNagSuppressionMeta(construct as CfnResource, [
        {
          id: "W12",
...
export class CfnNagResourcePathEndingWithSuppressionAspect implements IAspect {
  ...
  public visit(construct: Construct): void {
    if (construct.node.path.endsWith(this.resourcePathEndingWith)) {
      addCfnNagSuppressionMeta(construct as CfnResource, this.rulesToSuppress);
    }
  }
}
...
export class CfnNagResourcePathRegexSuppressionAspect implements IAspect {
  ...
  public visit(construct: Construct): void {
    if (construct.node.path.match(this.resourcePathEndingWith)) {
      addCfnNagSuppressionMeta(construct as CfnResource, this.rulesToSuppress);
    }
  }
}
