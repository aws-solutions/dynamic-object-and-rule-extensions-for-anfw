...
@injectable()
export class UpdateRuleBundleInputValidator extends InputValidator<UpdateRuleBundleInput> {
    logger: Logger;
    constructor(
        ...
        super();
        this.logger = loggerFactory.getLogger('UpdateRuleBundleInputValidator');
    }
    protected async validate(input: UpdateRuleBundleInput): Promise<void> {
        if (this.isBlank(input.ruleGroupArn)) {
            this.errors.push('ruleGroupArn cannot be null or empty.');
        }

        if (this.isBlank(input.id) || !this.isValidId(input.id)) {
        ...
        if (!(await this.generalValidator.isValidRuleGroup(input.ruleGroupArn))) {
            this.logger.info('invalidAggregator');
        ...
        if (!(await this.generalValidator.isValidAggregator(input.aggregatorName))) {
            this.logger.info('in isValidAggregator');
