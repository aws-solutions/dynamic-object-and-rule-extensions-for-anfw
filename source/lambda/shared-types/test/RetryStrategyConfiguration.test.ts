...
import {
  ...
  RETRY_COST,
  TIMEOUT_RETRY_COST,
} from "src/RetryStrategyConfiguration";

const INITIAL_RETRY_TOKENS = 100;
...
describe("Test exponentialBackOffDelayDecider", () => {
  test("should back off with jitter", () => {
    const waitTime1 = exponentialBackOffDelayDecider(10, 1);

    const waitTime2 = exponentialBackOffDelayDecider(10, 2);
    expect(waitTime1).not.toEqual(waitTime2);
  });
});
